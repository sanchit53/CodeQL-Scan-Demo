...
- PR workflow file: `.github/workflows/codeql-java-incremental.yml`.
- Sunday baseline workflow file: `.github/workflows/codeql-java-sunday-baseline.yml`.
...
- Pull requests and pushes in the incremental workflow are path-filtered to Java source, Gradle metadata, CodeQL configuration, and the workflow itself; XML-only changes such as `pom.xml` are intentionally left to the scheduled baseline workflow.
...
- The Sunday baseline workflow refreshes the default-branch baseline and keeps query coverage current, including XML-only metadata changes.
...
1. Merge both workflows to the default branch and run the Sunday baseline workflow once with `workflow_dispatch`. This creates the default-branch baseline database/cache that later pull requests can reuse.
...
6. Repeat with a PR that changes `settings.gradle` or shared build logic. Expect a broader re-extraction because dependency metadata can affect multiple modules. XML-only `pom.xml` changes should be covered by the Sunday baseline workflow rather than the incremental PR workflow.
